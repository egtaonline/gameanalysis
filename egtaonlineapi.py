...
class egtaonline(object):
    ...
    def get_scheduler(self, scheduler_name, verbose=False):
        ...
        if isinstance(scheduler_name, int):
            ...
            resp = self.request('get', 'schedulers/%d.json' % scheduler_name, data)
            resp.raise_for_status()
    ...
    def create_generic_scheduler(self, simulator_id, name, active, process_memory, size,
                                 time_per_observation, observations_per_simulation,
                                 nodes, default_observation_requirement, configuration):
        # pylint: disable=too-many-arguments
        '''Creates a generic scheduler and returns it's id

        simulator_id   - The id of the simulator for which you wish to make a scheduler.
        name           - The name for the scheduler.
        active         - True or false, specifying whether the scheduler is initially
                         active.
        process_memory - The amount of memory in MB that your simulations need.
        size           - The number of players for the scheduler.
        time_per_observation - The time you require to take a single observation.
        observations_per_simulation - The number of observations to take per simulation
                         run.
        nodes          - The number of nodes required to run one of your simulations.
        default_observation_requirement - The number of observations to take of a
                         profile in the absence of a specific request.
        configuration  - A dictionary representation that sets all the run-time parameters
                         for this scheduler.
    ...
    def generic_scheduler_add_role(self, scheduler_id, role, count):
        ...
        resp = self.request('post', 'generic_schedulers/%d/add_role.json' % scheduler_id,
    ...
    def get_game(self, game_name, granularity='structure'):
        ...
        if isinstance(game_name, int):
            ...
            resp = self.request('get', 'games/%d.json' % game_name,
