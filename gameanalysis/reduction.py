...
# FIXME This can maybe be fixed by making these all static classes that act
# like modules and all expose the same api
...
def _expand_rsym_profiles(game, profiles, full_players, reduced_players):
    ...
    assert np.all(np.add.reduceat(profiles, game.role_starts, 1) ==
                  reduced_players), \
    ...
    rep_red_players = np.maximum(
        reduced_players, 1).repeat(game.num_role_strats, -1)
    rep_full_players = full_players.repeat(game.num_role_strats, -1)
    num_profs = profiles.shape[0]
    expand_profs = profiles * rep_full_players // rep_red_players
    unassigned = full_players - \
        np.add.reduceat(expand_profs, game.role_starts, 1)

    # Order all possible strategies to find which to increment
    role_order = np.broadcast_to(game.role_indices,
                                 (num_profs, game.num_strats))
    error = profiles * rep_full_players / rep_red_players - expand_profs
    alpha_inds = np.arange(game.num_strats)
    alpha_ord = np.broadcast_to(alpha_inds, (num_profs, game.num_strats))
    ...
    rectified_inds = (inds + np.arange(num_profs)[:, None] *
                      game.num_strats)
    ind_mask = (np.arange(game.num_strats) <
                np.repeat(game.role_starts + unassigned, game.num_role_strats,
                          1))
...
def _reduce_rsym_profiles(game, profiles, full_players, reduced_players):
    ...
    rep_red_players = reduced_players.repeat(game.num_role_strats, -1)
    rep_full_players = np.maximum(
        full_players, 1).repeat(game.num_role_strats, -1)
    ...
    overassigned = np.add.reduceat(
        red_profs, game.role_starts, 1) - reduced_players
    ...
    role_order = np.broadcast_to(game.role_indices,
                                 (num_reduceable, game.num_strats))
    alpha_inds = np.arange(game.num_strats)
    ...
    rectified_inds = (inds + np.arange(num_reduceable)[:, None] *
                      game.num_strats)
    ind_mask = (np.arange(game.num_strats) < np.repeat(
        game.role_starts + overassigned, game.num_role_strats, 1))
...
class _Reduction(rsgame.StratArray, metaclass=abc.ABCMeta):
    ...
    def __init__(self, full_game, red_game):
        super().__init__(full_game.num_role_strats)
        assert np.all(full_game.num_strats == red_game.num_strats), \
            "Full reduced games must have same number of strategies"
...
class Hierarchical(_Reduction):
    ...
    def reduce_game(self, game, allow_incomplete=False):
        ...
        assert (np.all(game.num_role_players ==
                       self.full_game.num_role_players) and
                np.all(game.num_role_strats == self.num_role_strats)),\
            "The games players don't match up with this reduction"

        if isinstance(game, rsgame.SampleGame):
            if game.num_profiles == 0:
                return rsgame.samplegame(self.red_game.num_role_players,
            ...
            for profs, pays in zip(
                ...
                red_profiles, mask = _reduce_rsym_profiles(
                    self.full_game, profs, self.full_game.num_role_players,
            ...
            return rsgame.samplegame(self.red_game.num_role_players,
                                     game.num_role_strats, profiles,
                                     sample_payoffs, False)
        elif isinstance(game, rsgame.Game):
            if game.num_profiles == 0:
                return rsgame.game(self.red_game.num_role_players,
                                   game.num_role_strats)

            profiles = game.profiles
            payoffs = game.payoffs
            red_profiles, mask = _reduce_rsym_profiles(
                self.full_game, profiles, self.full_game.num_role_players,
                self.red_game.num_role_players)

            return rsgame.game(self.red_game.num_role_players,
                               game.num_role_strats, red_profiles,
                               payoffs[mask], False)

        else:
            return rsgame.basegame(self.red_game.num_role_players,
                                   game.num_role_strats)
    ...
    def reduce_profiles(self, profiles):
        ...
        return _reduce_rsym_profiles(
            self.full_game, np.asarray(
                profiles, int), self.full_game.num_role_players,
            self.red_game.num_role_players)[0]

    def expand_profiles(self, profiles):
        """Expand a set of profiles"""
        return _expand_rsym_profiles(self.full_game, np.asarray(profiles, int),
                                     self.full_game.num_role_players,
                                     self.red_game.num_role_players)
...
class DeviationPreserving(_Reduction):
    ...
    def _devs(self, players, num_profs):
        ...
        return np.tile(np.repeat(
            players - np.eye(self.full_game.num_roles, dtype=int),
            self.num_role_strats, 0), (num_profs, 1))
    ...
    def expand_profiles(self, profiles, return_contributions=False):
        ...
        num_profs = profiles.shape[0]
        dev_profs = profiles[:, None] - np.eye(self.full_game.num_strats,
                                               dtype=int)
        dev_profs = np.reshape(dev_profs, (-1, self.full_game.num_strats))
        dev_full_players = self._devs(
            self.full_game.num_role_players, num_profs)
        dev_red_players = self._devs(self.red_game.num_role_players, num_profs)

        mask = ~np.any(dev_profs < 0, 1)
        devs = np.eye(self.full_game.num_strats, dtype=bool)[None]\
            .repeat(num_profs, 0)\
            .reshape((-1, self.full_game.num_strats))[mask]
    ...
    def reduce_profiles(self, profiles, return_contributions=False):
        ...
        devs = np.eye(self.full_game.num_strats, dtype=int)[None]\
            .repeat(num_profs, 0)\
            .reshape((-1, self.full_game.num_strats))[mask][reduced]
        red_profs += devs
        ...
        if not return_contributions:
            ...
            full_inds = np.arange(num_profs)[:, None].repeat(
                self.full_game.num_strats, 1).flat[mask][reduced]
    ...
    def reduce_game(self, game, allow_incomplete=False):
        ...
        if isinstance(game, rsgame.SampleGame):
            ...
            rprof_index = red_inds * game.num_strats + strat_inds
            ...
            prof_inds = red_inds * game.num_strats + strat_inds
            ...
            for (prof_list, payoff_list), num_samples, offset in zip(
                ...
                num_profiles = prof_inds[-1] // game.num_strats + 1
                # With all payoffs and indices, put together sample payoffs
                # array
                sample_pays = np.bincount(
                    payoff_inds.flat, payoffs.flat,
                    num_profiles * game.num_strats * num_samples)\
                    .reshape(num_profiles, game.num_strats, num_samples)
                if allow_incomplete:
                    prof_offset = offset // game.num_strats
            ...
            return rsgame.samplegame(self.red_game.num_role_players,
                                     game.num_role_strats, new_profiles,
                                     sample_payoffs, False)
        elif isinstance(game, rsgame.Game):
            ...
            cum_inds = red_inds * game.num_strats + strat_inds
            ...
            return rsgame.game(self.red_game.num_role_players,
                               game.num_role_strats, red_profiles, red_payoffs,
                               False)
    ...
    def expand_deviation_profiles(self, subgame_mask, role_index=None):
        ...
        rdev = np.eye(self.num_roles, dtype=int)
        support = np.add.reduceat(subgame_mask, self.role_starts)
        ...
        if role_index is None:
            ...
            full_players = self.full_game.num_role_players.copy()
            full_players[role_index] -= 1
            red_players = self.red_game.num_role_players.copy()
            red_players[role_index] -= 1
            mask = np.split(subgame_mask, self.role_starts[1:])[role_index]
...
class Identity(_Reduction):
    ...
    def __init__(self, num_strats, num_role_players, num_role_players_=None):
        assert num_role_players_ is None or np.all(
            num_role_players == num_role_players_)
        game = rsgame.basegame(num_role_players, num_strats)
