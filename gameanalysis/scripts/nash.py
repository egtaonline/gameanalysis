...
def update_parser(parser):
    ...
    parser.add_argument('--regret', '-r', metavar='<thresh>', type=float,
            default=1e-3, help="""Max allowed regret for approximate Nash
            equilibria; default=1e-3""")
    parser.add_argument('--distance', '-d', metavar='<distance>', type=float,
            default=1e-3, help="""L2-distance threshold to consider equilibria
            distinct; default=1e-3""")
    parser.add_argument('--convergence', '-c', metavar='<convergence>',
            type=float, default=1e-8, help="""Replicator dynamics convergence
            thrshold; default=1e-8""")
    parser.add_argument('--max-iterations', '-m', metavar='<iterations>',
            type=int, default=10000, help="""Max replicator dynamics
    ...
    parser.add_argument('--type', '-t', choices=('mixed', 'pure', 'mrp'),
            ...
            compute: role-symmetric mixed-strategy Nash, pure-strategy Nash, or
            min-regret profile; default=mixed""")
    parser.add_argument('--random-points', '-p', metavar='<num-points>', type=int,
            default=0, help="""Number of random points from which to initialize
            replicator dynamics in addition to the default set of uniform and
            heavily-biased mixtures; default=0""")
    parser.add_argument('--one', '-n', action='store_true', help="""Always
    report at least one equilibrium per game. This will return the minimum
    regret equilibrium found, regardless of whether it was below the regret
    threshold""")
